...
class Environment:
    ...
    def apply_action(self, action):
        ...
        #self._curr_bbox = np.add(self._curr_bbox, action)
        self._curr_bbox = action*224
        #print(self._curr_bbox, action)
        ...
        try:
            if len(self._curr_bbox) == 1:
                self._curr_bbox = self._curr_bbox[0]
            ...
            print("prev", self._prev_bbox)
            print("action", action)
            print("curr", self._curr_bbox)
            raise e

        #is_valid = self._is_valid(self._curr_bbox) and self._is_inside(self._curr_bbox)
        is_valid = self._is_valid(self._curr_bbox) 
    ...
    def step(self, action, verbose=True):
        ...
        if not is_valid:
            #reward          = -1
            episode_done    = True
            fmt_str         = f"done(2:no area) (reward={reward})"

        if self._step >= self.max_steps:
            episode_done = True
            fmt_str      = f"done(3:max steps) (reward={reward})"
